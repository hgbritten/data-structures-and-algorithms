...
const count = (target, input) => {
  ...
  let value = newArr.filter(function (number) {
...
const totalSum = (input) => {
  let newArr = input.flat();
  let answer = newArr.reduce((acc, cur) => {
    acc = acc + cur;
    return acc;
  })
  return answer;
...
const divisibleByFiveTwoToThePower = (input) => {
  let divisibleByFive = input.map(value => value.filter(n => n % 5 == 0));
  let onlyNumbers = divisibleByFive.map(v => v.filter(n => typeof n === 'number'));
...
let findMaleAndFemale = (data) => {
  return data.filter(char => char.gender === 'male' || char.gender === 'female').map(i => i.name).join(' and ');
}
